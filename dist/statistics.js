...
const MUTATION_TYPES = ['minor', 'major', 'duplication', 'suppression', 'macro'];
export class Statistics {
    ...
    reset() {
        this.history = {
            ...
            mutations: Object.fromEntries(MUTATION_TYPES.map((k) => [k, []])),
        };
    ...
    recordDeath(organism) {
        ...
        if (this._deaths.length > this._deathWindow)
            this._deaths.shift();
    }
    recordMutation(tally) {
        for (const k of MUTATION_TYPES)
            this._mutationAccum[k] += tally[k] || 0;
    }
