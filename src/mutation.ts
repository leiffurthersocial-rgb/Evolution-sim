...
/** A temporary expression modifier to install on a child's genome. */
...
/** Result of mutating a parent genome. */
 ...
 */
const NAIVE_BENEFIT_DIRECTION: Record<GeneKey, number> = {
  strength: +1,
  speed: +1,
  size: -1,
  vision: +1,
...
export class MutationEngine {
  constructor(private rng: RNG, private config: SimConfig) {}

  /** Produce a mutated copy of `parentGenes` plus temporary modifiers. */
  mutate(parentGenes: Genes): MutationResult {
    const m = this.config.mutation;
    const genes: Genes = { ...parentGenes };
    ...
    for (const key of GENE_KEYS) {
      const g: GeneSpec = GENES[key];
      if (g.mutable === false) continue;
      if (!this.rng.chance(m.rate)) continue;
  ...
  private _applyDirectionalBias(key: GeneKey, delta: number): number {
    const m = this.config.mutation;
    const total = m.pBeneficial + m.pHarmful + m.pNeutral || 1;
    const r = this.rng.next() * total;
    const dir = NAIVE_BENEFIT_DIRECTION[key] || 1;
    const mag = Math.abs(delta);
    if (r < m.pBeneficial) return mag * dir;
    if (r < m.pBeneficial + m.pHarmful) return mag * -dir;
    return delta;
  }
