...
export class Renderer {
  ...
  render(): void {
    ...
    ctx.setTransform(
      this.dpr * cam.scale, 0, 0, this.dpr * cam.scale,
      this.dpr * cam.offsetX, this.dpr * cam.offsetY
    );
  ...
  private drawFood(): void {
    ...
    for (const f of this.sim.env.food) {
      if (f.eaten) continue;
      if (f.x < b.x0 || f.x > b.x1 || f.y < b.y0 || f.y > b.y1) continue;
  ...
  private drawOrganisms(): void {
    ...
    const margin = 20;
    const showText =
      this.overlays.age || this.overlays.fitness || this.overlays.generation;
    for (const o of this.sim.organisms) {
      if (o.x < b.x0 - margin || o.x > b.x1 + margin || o.y < b.y0 - margin || o.y > b.y1 + margin) {
        continue;
      }
      this.drawOrganism(o);
      if (showText) this.drawOrganismOverlay(o);
    }
    ...
    if (this.hoveredId !== null && this.hoveredId !== this.selectedId) {
      ...
      if (o) this.drawHighlight(o, 'rgba(255,255,255,0.5)', 2);
    }
    if (this.selectedId !== null) {
      const o = this.sim.findById(this.selectedId);
      if (o) this.drawSelection(o);
    }
  }
  ...
  private drawSelection(o: Organism): void {
    const ctx = this.ctx;
  ...
  private drawOrganismOverlay(o: Organism): void {
    const ctx = this.ctx;
    ...
    if (label !== null) ctx.fillText(label, o.x, o.y - o.radius - 3 / this.camera.scale);
  }
   ...
   */
  pick(worldX: number, worldY: number): Organism | null {
