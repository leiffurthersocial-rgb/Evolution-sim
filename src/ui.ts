...
import { GENE_KEYS, GENES, GeneKey, SimConfig, SPEED_PRESETS } from './config.js';
import { PREFERENCE_KEYS } from './selection.js';
import { clamp, norm } from './utils.js';
import type { Renderer, Overlays, ColorMode } from './renderer.js';
...
export class UI {
  ...
  private buildDisplayControls(): void {
    ...
    sel.addEventListener('change', () => this.deps.onColorMode(sel.value as ColorMode));
  }
  ...
  setTps(tps: number): void {
    ...
    if (el) el.textContent = tps >= 100 ? Math.round(tps).toString() : tps.toFixed(1);
  }
   ...
   */
  updateInspector(o: Organism | null, following = false): void {
  ...
  private nearestSpeedIndex(speed: number): number {
    let best = 0;
    let bestD = Infinity;
